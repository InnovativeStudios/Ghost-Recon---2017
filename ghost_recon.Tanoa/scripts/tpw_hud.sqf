...
tpw_hud_fnc_goggles =
	...
	if ((((goggles _unit) in tpw_hud_goggletypes) || ((headgear _unit) in tpw_hud_headgeartypes)) && {cameraview != "external" || tpw_hud_thirdperson == 1} && {(eyepos player) select 2 > 0}) then
		{
...
tpw_hud_fnc_ping =
	...
	while {true} do
		{
		if (tpw_hud_active && {tpw_hud_ping == 1}) then
			{
...
tpw_hud_fnc_unitscan =
	{
	private ["_ctg","_colour","_nearunits","_nearveh","_side","_icon","_hidicon","_ctz","_cty","_unit","_farunits","_farveh"];
	...
	if (cameraView == "gunner") then
		{
		_farunits = (getpos player) nearentities [["camanbase"],tpw_hud_maxrange * 3]; // units spotted further out
		_farveh = (getpos player) nearentities [["landvehicle","air"],tpw_hud_vehiclerange * 3]; // vehicles spotted further out
		for "_ctz" from 0 to (count _farunits - 1) do 
			{
			_unit = _farunits select _ctz;
			_unitpos = getpos _unit;
			if (player distance _unit > tpw_hud_maxrange) then
				{
				_pos = getpos player;
				_ang = ((((_unitpos select 0) - (_pos select 0)) atan2 ((_unitpos select 1) - (_pos select 1))) + 360) % 360; // heading of unit from player
				if (_ang > 360) then 
					{
					_ang = _ang - 360
					};
				if ((_ang > direction player - 2) and (_ang < direction player + 2)) then // unit must be within 2 deg of the player's direction
					{
					if (_unit getvariable["tpw_hud_fartime",0] < time) then
						{
						tpw_hud_farunits set [count tpw_hud_farunits,_unit];
						};
					_unit setvariable["tpw_hud_fartime", time + 30]	;		
					};
				};
			}; 
		for "_ctz" from 0 to (count _farveh - 1) do 
			{
			_unit = _farveh select _ctz;
			_unitpos = getpos _unit;
			if (player distance _unit > tpw_hud_maxrange) then
				{
				_pos = getpos player;
				...
				if ((_ang > direction player - 2) and (_ang < direction player + 2)) then // unit must be within 2 deg of the player's direction
					...
					if (_unit getvariable["tpw_hud_fartime",0] < time) then
						{
						tpw_hud_farvehicles set [count tpw_hud_farvehicles,_unit];
						};
					_unit setvariable["tpw_hud_fartime", time + 30]	;
					};
				};
	...
	for "_cty" from 0 to (count tpw_hud_farunits -1) do
		{
		_unit = tpw_hud_farunits select _cty;
		if (_unit getvariable "tpw_hud_fartime" < time) then
			{
			tpw_hud_farunits set [_cty,-1];
			};
		};
	tpw_hud_farunits = tpw_hud_farunits - [-1];	
	_nearunits = _nearunits + tpw_hud_farunits;
	
	for "_cty" from 0 to (count tpw_hud_farvehicles -1) do
		{
		_unit = tpw_hud_farvehicles select _cty;
		if (_unit getvariable "tpw_hud_fartime" < time) then
			{
			tpw_hud_farvehicles set [_cty,-1];
			};
		};
	tpw_hud_farvehicles = tpw_hud_farvehicles - [-1];	
	...
	for "_ctg" from 0 to (count _nearunits - 1) do	
		...
		if (tpw_hud_hideciv && {side _unit == CIVILIAN}) then
			{
			_nearunits set [_ctg,-1];
			};	
		};	
