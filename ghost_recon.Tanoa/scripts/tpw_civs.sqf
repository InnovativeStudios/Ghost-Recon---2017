...
tpw_civ_fnc_civspawn =
	...
	// Only bother if houses nearby
	if (count tpw_civ_houses < 2) exitwith {};
		...
		if (count tpw_civ_civarray > 0) then
			...
			for "_i" from 0 to (count tpw_civ_civarray - 1) do
				...
				if !(alive _civ) then 
					...
					if ((_civ distance vehicle player > tpw_civ_radius) && {(lineintersects [eyepos player,getposasl _civ,player,_civ]) || (terrainintersectasl [eyepos player,getposasl _civ])}) then
						{
