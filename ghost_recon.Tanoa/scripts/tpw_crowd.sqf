...
if (count _this < 6) exitwith {hint "TPW CROWD incorrect/no config, exiting."};
...
tpw_crowd_fnc_spawn =
	{	
	private ["_house","_roads","_civ","_civtype","_spawnpos","_move","_animct","_sel"];
	...
	for "_i" from 1 to (2 + floor random 1) do
		...
		_civ = tpw_crowd_sqname createUnit [_civtype,_spawnpos, [], 0, "FORM"]; 
		_civ setpos (position _civ); // make sure they're at ground level
		...
		_civ setdir ([_civ, _spawnpos] call BIS_fnc_dirTo);
...
while {true} do 
	...
	if (tpw_crowd_active) then
		...
		for "_i" from 0 to count tpw_crowd_array - 1 do
			...
			_mov = _civ getvariable "tpw_crowd_move";			
