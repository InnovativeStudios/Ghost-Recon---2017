...
//ASL - ADVANCED SQUAD LEADER - AI SQUAD LEADER CAN CALL IN CAS OR ARTILLERY ON HIGH VALUE TARGETS
tpw_skirmish_fnc_asl =
	...
	_fnc_casheli =
		...
		_heli2 = [_startpos2,140,_helitype,_grp] call BIS_fnc_spawnVehicle; // Spawn helicopter2 and crew
	...
	_fnc_target =
		...
		for "_i" from 0 to (count _allunits - 1) do 
			...
			{!(lineintersects [position _unit,position _sql]) || (_sql knowsabout _unit > 0)} // Squad leader must either see unit or know about it
			) then
				{
				// Enemies			
...
//ENEMY ASL - ADVANCED SQUAD LEADER - OPFOR AI SQUAD LEADER CAN CALL IN CAS OR ARTILLERY ON HIGH VALUE TARGETS
...
//RESISTANCE ASL - ADVANCED SQUAD LEADER - INDFOR AI SQUAD LEADER CAN CALL IN CAS OR ARTILLERY ON HIGH VALUE TARGETS
...
tpw_skirmish_fnc_roads =
	...
	if (player distance tpw_skirmish_lastroadpos > (tpw_skirmish_maxspawnradius / 2)) then 
		...
		tpw_skirmish_roads = (player nearroads tpw_skirmish_maxspawnradius) select {isonroad _x};
		};
	};
...
tpw_skirmish_fnc_friendlywp = 	
	{
	private ["_ct","_group","_killpos","_i","_formations","_form"];
	...
	for "_ct" from 0 to (count tpw_skirmish_friendlysquads - 1) do	
		...
		if (!isnil "_group") then
			...
			_formations = ["COLUMN", "STAG COLUMN", "WEDGE", "ECH LEFT", "ECH RIGHT", "VEE","LINE","DIAMOND","FILE"];
			_form = _formations select floor random count _formations;
			[(leader _group),_killpos, 250, 7, "MOVE","SAFE", "YELLOW", "LIMITED",_form, "_this spawn CBA_fnc_searchNearby", [3,6,9]] call CBA_fnc_taskPatrol; 
			};
		};
	tpw_skirmish_friendlywpflag = true;	
	};	
...
tpw_skirmish_fnc_enemywp = 	
	{
	private ["_ct","_group","_killpos","_formations","_form"];
...
tpw_skirmish_fnc_resistwp = 	
	{
	private ["_ct","_group","_killpos","_formations","_form"];
	tpw_skirmish_resistwpflag = false; // only one instance of this function can run at once
	_killpos = _this select 0;
	for "_ct" from 0 to (count tpw_skirmish_resistsquads - 1) do	
		{
		sleep random 2;
		_group = tpw_skirmish_resistsquads select _ct;
		if (!isnil "_group") then
			{
			// Get squads to move towards and patrol around killed unit	
			_formations = ["COLUMN", "STAG COLUMN", "WEDGE", "ECH LEFT", "ECH RIGHT", "VEE","LINE","DIAMOND","FILE"];
...
tpw_skirmish_squadspawn = 
	...
	if (count tpw_skirmish_houses > 10 && {random 100 > 50}) then
		{
		_house = tpw_skirmish_houses select (floor (random (count tpw_skirmish_houses)));
		_spawnpos = position _house;
		} else
		{
		_spawnpos = [position player, tpw_skirmish_minspawnradius, tpw_skirmish_maxspawnradius, 3, 0, 100, 0] call BIS_fnc_findSafePos;
		};
	...
	for "_i" from 0 to _max do 
		...
		_unit = _unit createUnit [_spawnpos,_sqname,"this enablesimulation false;this setvariable ['tpw_skirmish',1];this setskill 0.05 + random 0.15;this setAnimSpeedCoef 0.8 + random 0.2"];		
		sleep random 1;
		};
	...
	_formations = ["COLUMN", "STAG COLUMN", "WEDGE", "ECH LEFT", "ECH RIGHT", "VEE","LINE","DIAMOND","FILE"];
	_form = _formations select floor random count _formations;
	[_leader, _leader, 250, 7, "MOVE", "SAFE", "GREEN", "LIMITED",_form, "_this spawn CBA_fnc_searchNearby", [3,6,9]] call CBA_fnc_taskPatrol; 
			...
			if (tpw_skirmish_resist_support == 1 && {_num in [0,6,-6]}) then
				...
				_x addeventhandler ["killed",
					...
					if (tpw_skirmish_resistwpflag) then
						...
						[position (_this select 0)] spawn tpw_skirmish_fnc_resistwp;
						};
					}];
				_x enablesimulation true;
				} foreach units _sqname;
...
tpw_skirmish_fnc_waypoint =
	...
	if (count tpw_skirmish_roads > 50) then
		...
		{
		_wppos = [getposasl player, tpw_skirmish_minspawnradius, tpw_skirmish_maxspawnradius, 3, 0, 100, 0] call BIS_fnc_findSafePos;
		};
...
tpw_skirmish_fnc_squadremove =
	...
	for "_ct" from 0 to (count _squads - 1) do	
		...
		_alive = (units _squad) select {alive _x}; 
...
tpw_skirmish_fnc_nocollide =
	...
	// Only run if TPW CARS isn't active
	if (!(isnil "tpw_car_active") && !(tpw_car_active)) then
		{
		while {true} do
			{
